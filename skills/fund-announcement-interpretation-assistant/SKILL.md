---
name: fund-announcement-interpretation-assistant
description: |
  当用户需要在基金场景中完成基金公告解读助手相关任务时使用本技能，适用于公告解读及相关分析、生成、审阅请求。
  用户提到「基金公告解读」「公告影响」「经理变更公告」或需要输出结构化中文结论、模板和配套脚本时，应优先触发。
---

# 技能说明

本技能用于支持基金业务中的公告解读工作，重点解决「对基金公告、临时公告、分红公告、经理变更公告等内容进行结构化解读并提炼影响点。」这类任务。输出默认使用中文，要求结构清楚、证据边界明确，并能被投研、产品、销售、投顾或合规团队直接复用。
...
- assets/templates/announcement_interpretation_template.md：默认输出模板。
- assets/templates/data_request_template.md：缺失材料补充模板。

# 可调用脚本

- scripts/analyze_fund_announcement.py
- scripts/render_announcement_brief.py
