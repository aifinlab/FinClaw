...
def summarize(records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {"overview": overview, "overdue": issues}
...
def render_markdown(summary: Dict[str, Any]) -> str:
    ...
    for item in summary["overdue"]:
        lines.append(f"- {item.get('defect_id', 'unknown')} | {item.get('title', '')} | {item.get('owner', '')}")
    return "\n".join(lines)


def main() -> None:
    parser = argparse.ArgumentParser(description="缺陷跟踪汇总")
