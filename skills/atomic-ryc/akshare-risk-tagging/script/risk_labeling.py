...
try:
    ...
    raise SystemExit("未安装 akshare，请先执行: pip install akshare pandas numpy") from exc
...
def build_labels(spot_row: pd.Series, hist_df: pd.DataFrame) -> RiskLabelResult:
    symbol = str(spot_row.get("symbol", ""))
    name = str(spot_row.get("name", ""))
    price = _to_float(spot_row.get("price"))
    daily_change_pct = _to_float(spot_row.get("pct_change"))
    turnover_rate = _to_float(spot_row.get("turnover_rate"))
...
def run_labeling(symbols: Optional[Sequence[str]], limit: Optional[int]) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        df = df.sort_values(["risk_score", "symbol"], ascending=[False, True]).reset_index(drop=True)
    return df
