---
name: global-keyword-retrieval
description: 用于关键词检索场景。适用于金融工作中的基础任务单元。
---

# 法律条文或公司报告关键词检索 Skill

针对法律条文与上市公司报告做关键词检索，支持短语和部分同义词扩展。
...
- 对法律条文和公司报告切分建库；
- 支持关键词、短语和基础同义词扩展；
- 返回命中词、来源标题、页码、URL 与证据片段。

## 使用示例
```bash
pip install -r requirements.txt
python script/fetch_public_data.py --data-dir data
python script/keyword_retrieval.py --corpus data/corpus.jsonl --query "风险 信息披露" --top-k 5
