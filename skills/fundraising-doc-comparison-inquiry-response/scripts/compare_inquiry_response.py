...
def compare_response(data: Dict[str, Any]) -> Dict[str, Any]:
    """对比问询回复"""
    results = {
        'items': [],
        'discrepancies': [],
    ...
    for item in items:
        ...
        response_value = item.get('inquiry_response', '')
        
        # 简单文本对比
        is_consistent = str(prospectus_value).strip() == str(response_value).strip()
...
def generate_report(results: Dict[str, Any], output_path: str) -> str:
    ...
    # 对比明细
    lines.append("## 对比明细\n")
    lines.append("| 对比项 | 招股书 | 问询回复 | 一致性 |")
    lines.append("|--------|--------|----------|--------|")
    for item in results['items']:
        icon = "✅" if item['is_consistent'] else "❌"
        lines.append(f"| {item['name']} | {item['prospectus']} | {item['response']} | {icon} |")
    lines.append("")
    ...
    if results['discrepancies']:
        ...
        for idx, disc in enumerate(results['discrepancies'], 1):
            ...
            lines.append(f"- 问询回复：{disc['response']}")
            lines.append(f"- 建议：核实差异原因，同步更新相关文件\n")
    
    # 结论
    lines.append("## 对比结论\n")
    if results['discrepancies']:
        lines.append(f"存在{len(results['discrepancies'])}项差异，需核实原因并同步更新。\n")
    else:
        lines.append("问询回复与招股书信息披露一致。\n")
    
    # 输出报告
    report_content = "\n".join(lines)
    
    output_file = Path(output_path) / f"问询回复对比报告_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def main():
    parser = argparse.ArgumentParser(description='问询回复对比工具')
