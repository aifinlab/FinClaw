...
def build_report(payload: Dict[str, object]) -> Dict[str, object]:
    return {
        "match_summary": payload.get(
            "match_summary", ""), "evidence": payload.get(
            "evidence", []), "false_positive_checks": payload.get(
                "false_positive_checks", [
                    "核对证件号/统一信用代码", "确认名单更新时间与版本"]), "pending_items": payload.get(
                        "pending_items", ["补充客户身份核验材料"]), "recommended_actions": payload.get(
                            "recommended_actions", ["必要时升级合规复核"]), }
