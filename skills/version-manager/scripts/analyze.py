...
def hit_count(text:str, kws:List[str])->int:
    c=0
    for kw in kws:
...
def main():
    ...
    for i,it in enumerate(items, start=1):
        ...
        ph=hit_count(text,pos); nh=hit_count(text,neg)
        score=max(0,min(100,60+ph*4-nh*6))
        level='high' if score<45 else 'medium' if score<70 else 'low'
        out.append({'id':it.get('id',f'R{i:03d}'),'score':score,'level':level,'ph':ph,'nh':nh})
    c=Counter(x['level'] for x in out)
    lines=[f"# {a.title}",'','## 一、概览',f"- 样本数: {len(out)}",f"- low: {c.get('low',0)}",f"- medium: {c.get('medium',0)}",f"- high: {c.get('high',0)}",'','## 二、重点条目']
