...
def evaluate(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    notes = []
    rd_ratio = record.get("rd_ratio")
    if rd_ratio:
        try:
            ratio = float(rd_ratio)
            if ratio < 0.03:
                notes.append("研发投入比例偏低")
        except ValueError:
            notes.append("研发投入比例格式异常")
    status = "需补件" if missing else "可进入下一步"
    return {"status": status, "missing_fields": missing, "risk_notes": notes}


def render_markdown(result: Dict[str, Any]) -> str:
    lines = ["# 科创准入预审结果", "", f"- 状态: {result['status']}"]
    lines.append("\n## 缺失字段")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="科创客户准入预审")
