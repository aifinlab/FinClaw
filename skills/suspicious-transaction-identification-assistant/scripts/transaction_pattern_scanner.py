...
def scan_patterns(transactions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    counterparties = [str(t.get("counterparty", "")).strip()
                      for t in transactions if str(t.get("counterparty", "")).strip()]
    ...
    if len(transactions) >= 20 and small_count / \
        ...
        signals.append(
            Signal(
                ...
                score=20,
                evidence=f"共 {
                    len(transactions)} 笔交易，其中 {small_count} 笔为小额交易，占比偏高。",
            ))

    if len(transactions) >= 10 and late_count / \
            max(len(transactions), 1) >= 0.3:
    ...
    if total_amount > 0 and max_amount / \
            total_amount >= 0.5 and len(transactions) >= 5:
        signals.append(Signal(
