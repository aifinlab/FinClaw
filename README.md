...
[![Skills](https://img.shields.io/badge/Skills-1000%2B-blueviolet.svg)](#-skills-全景目录)
...
FinClaw 是首个面向金融行业的开源系统执行框架，由上海财经大学统计与数据科学学院、上海金融智能工程技术研究中心主任张立文教授领衔的人工智能金融实验室（AIFinLab）联合研发。FinClaw 不是一个泛泛而谈的通用 Agent，而是按金融行业垂直拆分出的六只专属龙虾。银行、证券、基金、保险、期货、信托，六大赛道，各有各的业务流程、监管逻辑和交付场景，也各有一只真正懂行的龙虾。
...
### 🦞 银行龙虾 | 对公授信+自营投资专属智囊
围绕授信审批、行业研究、资产配置等核心场景，快速生成行业信用风险监测清单、自营账户大类资产配置思路等专业结果，把大量重复性的前期分析工作先做起来。

### 🦞 基金龙虾 | 净值归因+业绩诊断精准手术刀
一键拆解ETF跑输基准的核心原因，定位拖累净值的重仓股，区分基本面/交易面/事件面影响，覆盖投研回测、绩效归因、持仓合规、FOF组合构建全链路。

### 🦞 证券龙虾 | 投研研判+业务拓展精准雷达
从投行尽调底稿、行业投研覆盖排序，到机构路演核心议题、两融业务测算，精准捕捉政策与市场主线，让投研与经纪业务直接踩中节奏。

### 🦞 保险龙虾 | 负债匹配+资产配置定制专家
结合险企负债结构、利率环境与风险偏好，生成更贴近实际业务场景的大类资产配置思路，同时覆盖产品对比、保障分析、核保理赔、合规风控等常见任务。

### 🦞 信托龙虾 | 非标业务+财富传承专属架构师
针对家族信托、高净值客户服务、非标估值、ABS测算、政信风控等场景，生成资产适配诊断、风险识别、受益分配框架与控制权安排建议，辅助完成更完整的方案设计。

### 🦞 期货龙虾 | 行情复盘+交易研判全天候操盘手
分钟级完成主力合约复盘，拆解日内拐点、量仓变化、跨期价差、产业驱动，精准定性尾盘波动是趋势延续、情绪宣泄还是短线错杀，量化与主观交易者均能开箱即用。

---

## 🌟 FinClaw 核心亮点

FinClaw 以金融业务全流程闭环为核心，从底层数据到上层应用形成完整链路。系统采用分层架构设计，将金融能力与任务执行进行解耦与重构。底层通过 Skills库 沉淀标准化的金融原子能力与任务能力，上层由统一的任务编排与调度机制对能力进行动态组合与调用，从而实现从"工具调用"向"任务执行"的跃迁。

### 1️⃣ 1000+ Skills 全品类覆盖

FinClaw 的 Skills 体系并不是按照金融行业简单拆分，而是从更底层的能力结构出发，对金融业务中的通用能力进行抽象与标准化沉淀。这些能力在实际使用中可以被灵活编排与组合，从而自然延展至银行、证券、基金、保险、期货、信托六大核心金融应用场景。
...
FinClaw 采用分层架构设计，将金融能力与任务执行进行解耦与重构，实现从"工具调用"向"任务执行"的跃迁。
...
│                              ▼                                               │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │                     Dr. Claw Research Lab (🚧待集成)                  │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐             │   │
│  │  │ Research │  │   Task   │  │  Skill   │  │  Auto    │             │   │
│  │  │   Lab    │  │ Pipeline │  │ Library  │  │ Research │             │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘             │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                      ② OpenClaw Gateway 平台层                              │
├─────────────────────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │                     多通道消息适配器 (Channel Router)                   │   │
│  │   飞书 Protocol  │  Discord Bot  │  Slack API  │  微信企业号  │        │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                                      │                                       │
│  ┌───────────────────────────────────┼───────────────────────────────────┐   │
│  │                         Agent 生命周期管理                            │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐             │   │
│  │  │ Session  │  │  Skill   │  │  Memory  │  │  Cron    │             │   │
│  │  │ Manager  │  │ Registry │  │  Manager │  │ Scheduler│             │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘             │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                                      │                                       │
│  ┌───────────────────────────────────┼───────────────────────────────────┐   │
...
│  │                      多数据源适配器 (Adapters)                        │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐             │   │
│  │  │ AkShare  │  │TusharePro│  │ 同花顺API│  │ 腾讯财经 │             │   │
│  │  │  (主要)  │  │ (财务)   │  │ (指标)   │  │ (实时)   │             │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘             │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐             │   │
│  │  │BaoStock  │  │Yahoo Fin │  │ Quandl  │  │ 智兔数服 │             │   │
│  │  │(历史)    │  │(美股)    │  │(宏观)    │  │(免注册)  │             │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘             │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                                      │                                       │
...
├─────────────────────────────────────────────────────────────────────────────┤
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │   LLM服务    │  │   文件存储   │  │   日志系统   │  │   监控告警   │      │
│  │ Kimi/Claude  │  │  Workspace   │  │  MEMORY.md   │  │   心跳检测   │      │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘      │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                         ① 交互层 (Interface)                              │
│  ┌──────────────┐                                                        │
│  │   飞书/CLI   │  ──→ 消息协议转换 ──→ OpenClaw Gateway                   │
│  │   消息入口   │                                                        │
│  └──────────────┘                                                        │
└──────────────────────────────────────────────────────────────────────────┘
                                    │
...
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │  Session Manager: 创建/恢复 Agent 会话                            │   │
│  │  ──→ 每个用户会话对应一个 Agent 实例                              │   │
│  └────────────────────────────────┬─────────────────────────────────┘   │
└───────────────────────────────────┼──────────────────────────────────────┘
                                    │
...
│  │                  ┌──────────────┐                                 │   │
│  │                  │ Skill        │                                 │   │
│  │                  │ Orchestrator │  ◄── 从 Registry 查询可用 Skills │   │
│  │                  │ (Skill调度)   │                                 │   │
│  │                  └──────┬───────┘                                 │   │
│  │                         │                                         │   │
│  │         ┌───────────────┼───────────────┐                         │   │
...
│  │                    Data Fetcher (统一接口)                        │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐         │   │
│  │  │ AkShare  │  │腾讯财经  │  │同花顺API │  │ 智兔数服 │         │   │
│  │  │ (主要)   │  │ (实时)   │  │ (财务)   │  │ (免注册) │         │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘         │   │
│  │                                                                  │   │
│  │  故障转移: 主源失败 ──→ 备用源 ──→ 备用源2 ──→ 模拟数据           │   │
│  └────────────────────────────────┬─────────────────────────────────┘   │
└───────────────────────────────────┼──────────────────────────────────────┘
                                    │
                                    ▼
...
| 组件 | 职责 | 对应文件 |
|:---|:---|:---|
| **Session Manager** | 创建/维护用户会话，绑定 Agent 实例 | OpenClaw Gateway |
| **Intent Engine** | 解析用户输入，识别金融意图 | intent_recognizer.py |
| **SOUL Engine** | 人格驱动决策，控制输出风格 | SOUL.md + 系统提示词 |
| **Memory Manager** | 读写记忆，加载用户偏好 | memory_search / memory_get |
| **Skill Orchestrator** | 调度 Skills 执行，管理依赖 | Agent 内部逻辑 |
| **Data Fetcher** | 多源数据获取，故障转移 | data_fetcher.py |
...
- ✅ **已完成**：1000+ 个 Skills（数据源/投研/量化/保险/银行/助手/工具）、统一数据抽象层、环境变量驱动路径、Docker 部署
...
诚邀业界同仁共同探索 AI 与金融深度融合的创新范式，共建智慧金融新生态，并通过邮件与 [zhang.liwen@shufe.edu.cn](mailto:zhang.liwen@shufe.edu.cn) 联系。
