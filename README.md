...
**FinClaw** 是首个面向金融行业的开源系统执行框架，由**上海财经大学统计与数据科学学院、上海金融智能工程技术研究中心主任张立文教授**领衔的人工智能金融实验室（AIFinLab）联合研发。FinClaw 不是一个泛泛而谈的通用 Agent，而是**按金融行业垂直拆分出的六只专属龙虾**。银行、证券、基金、保险、期货、信托，六大赛道，各有各的业务流程、监管逻辑和交付场景，也各有一只真正懂行的龙虾。
...
Uploading FinClaw演示-基金龙虾精选案例.mov…
...
FinClaw 基于 OpenClaw 原生 AI Agent 框架，采用五层技术架构构建金融业务全流程闭环：
- 用户交互层：飞书/微信/CLI等多端接入；
- OpenClaw核心层：消息路由、Session管理、Skill注册与权限控制；
- FinClaw业务层：SOUL人格引擎 + 意图识别+Skill调度+记忆管理；
- .数据层：统一接口+多源适配（AkShare/同花顺等）+ 故障转移；
- 基础设施层：LLM服务、文件存储、日志监控。

**通过五层架构实现「用户输入→意图识别→技能调度→数据获取→格式化输出」的高效数据流。**
